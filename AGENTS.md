...
2. **Force-rebuild ALL guest assets** (auto mode skips existing files):
   ```bash
   # Local build
   OPENVM_RUST_TOOLCHAIN=nightly-2026-03-17 cargo run --release -p scroll-zkvm-build-guest -- --mode force

   # Docker build (matches CI)
   OPENVM_RUST_TOOLCHAIN=nightly-2026-03-17 make build-guest
   ```
...
**Symptoms**: Errors involving duplicate `alloy`/`revm` types or APIs expected by the stateless block verifier.
**Cause**: This fork intentionally pins Alloy, REVM, and the DogeOS SBV revision as one compatible dependency set. A global update can still churn transitive packages outside those exact pins and make the lockfile incompatible with the Reth/SBV stack.
**Fix**: Restore the original `Cargo.lock` and refresh only the OpenVM git sources needed by the upgrade.
...
|-----------|---------------------------------------------|---------------------------------------------|
| `AggregationTreeConfig` | `num_children_internal: 2, num_children_leaf: 4` | Same |
